...
func _on_hitbox_enemy_2_body_entered(body):
	if body.is_in_group("Player"):
		...
		swoop_speed = 0
		health -= 20
		print("emit -> playerHealthDamaged 40")
		Signals.emit_signal("playerHealthDamaged", 40)

func _on_hitbox_enemy_2_body_exited(body):
	if body.is_in_group("Player"):
